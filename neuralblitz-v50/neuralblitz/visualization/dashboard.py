...
class DashboardMetrics:
    ...
    def clear_old_alerts(self, hours: int = 1):
        ...
        cutoff = datetime.now(tz=__import__("datetime").timezone.utc) - timedelta(hours=hours)
        self.active_alerts = [
            alert
...
class NeuralBlitzDashboard:
    ...
    def _create_processing_rate_graph(self) -> dict:
        ...
        recent_times = [
            datetime.now(tz=__import__("datetime").timezone.utc) - timedelta(seconds=i) for i in range(30, 0, -1)
        ]
        rates = [
