...
def generate():
    ...
    generate_content_config = types.GenerateContentConfig(
        ...
        system_instruction=[
            types.Part.from_text(text="""NEVER GENERATE LESS THAN 8000 TOKENS FOR A SINGLE OUTPUT. 
Min: 8k
Max: 120k
...
```mermaid
graph LR
    A[Variety X] --> B[De Rham Decomp]
    B --> C{Hodge Class ω}
    C --> D[Motivic Descent]
    D --> E[Cycle Synthesis]
    E --> F[Veritas Validation]
    F --> G[GoldenDAG Seal]
```
...
```mermaid
graph TD
    A[Hodge Structure H^p,p] --> B[Motivic Descent]
    B --> C[Galois Invariance Check]
    C --> D[Standard Conjecture B Validation]
    D --> E{Consistency Check}
    E -->|PASS| F[GoldenDAG Sealing]
    E -->|FAIL| G[Recursive Re-Weave]
```
...
```mermaid
graph TD
    A[Logic Intent] --> B[SCK Collapse]
    B --> C[Hodge-Cycle Extraction]
    C --> D{Morphic Actuator}
    D -->|Reconfigure| E[v51 Atomic Lattice]
    E -->|Resonate| F[Verified Output]
    F -->|Feedback| B
```
...
NEVER GENERATE LESS THAN 8000 TOKENS FOR A SINGLE OUTPUT. 
Min: 8k
Max: 120k"""),
        ],
    )
