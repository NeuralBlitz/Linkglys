...
class RoleBasedRateLimiter:
    ...
    def check_rate_limit(self, key_id: str, endpoint: str) -> Dict[str, Any]:
        ...
        if api_key.expires_at and datetime.now(datetime.timezone.utc) > api_key.expires_at:
    ...
    def record_api_usage(self, key_id: str, endpoint: str, success: bool = True):
        ...
        if api_key:
            ...
            api_key.last_used = datetime.now(datetime.timezone.utc)
...
class ThreatDetection:
    ...
    def __init__(self):
        self.suspicious_patterns = {
            ...
            "sql_injection": [
        r"(\b[\x27\x22](\w|;|--)|(?i)(\b(union|select|insert|delete|update|drop|create|alter)))",
