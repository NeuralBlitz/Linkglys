...
def create_classifier(model_path: Optional[str] = None) -> IntentClassifier:
    ...
    def create_synthetic_training_data(self, n_samples: int = 500) -> tuple:
        ...
        # Generate feature vectors for 5 categories
        n_features = 6  # confidence, urgency, complexity, familiarity, clarity, context
        n_classes = 5
        ...
        for class_idx in range(n_classes):
            ...
            centers = [
                [0.8, 0.3, 0.2, 0.9, 0.9, 0.8],  # greeting
                [0.6, 0.5, 0.7, 0.4, 0.5, 0.6],  # question
                [0.7, 0.8, 0.6, 0.5, 0.6, 0.5],  # action
                [0.5, 0.4, 0.5, 0.3, 0.4, 0.7],  # navigation
                [0.9, 0.9, 0.8, 0.2, 0.3, 0.4],  # system
            ]
            center = centers[class_idx]
            for _ in range(samples_per_class):
                sample = [max(0, min(1, c + rng.normal(0, 0.15))) for c in center]
                X.append(sample)
                y.append(class_idx)
...
def demo_intent_classifier():
    ...
    test_intents = [
        [0.8, 0.7, 0.9, 0.5, 0.6, 0.4],  # Creative
