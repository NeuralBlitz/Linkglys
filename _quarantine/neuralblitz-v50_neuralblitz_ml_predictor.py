...
class ConsciousnessPredictor:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        self.last_retrain = datetime.now(datetime.timezone.utc)
    ...
    def _extract_features(self, historical_data_sample: Dict[str, Any]) -> np.ndarray:
        ...
        try:
            ...
            recent_times = [d.get("timestamp", datetime.now(datetime.timezone.utc)) for d in historical_data_sample if "timestamp" in d]
            request_count = len([t for t in recent_times if (datetime.now(datetime.timezone.utc) - t).total_seconds() < 300])  # Last 5 minutes
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_retrain": (self.last_retrain + timedelta(hours=self.retrain_interval_hours)).isoformat() if self.last_retrain else (datetime.now(datetime.timezone.utc) + timedelta(hours=self.retrain_interval_hours)).isoformat(),
    ...
    def should_retrain(self) -> bool:
        ...
        time_since_retrain = datetime.now(datetime.timezone.utc) - self.last_retrain
