...
class EventAggregator:
    ...
    def __init__(self, window_size_minutes: int = 60):
        ...
        self.last_aggregation = datetime.now(datetime.timezone.utc)
    ...
    def add_event(self, event: AnalyticsEvent):
        ...
        if datetime.now(datetime.timezone.utc) - self.last_aggregation > timedelta(minutes=5):
...
class AnomalyDetector:
    ...
    async def check_for_anomalies(self, metrics: Dict[str, Any]):
        ...
        for anomaly in anomalies:
            anomaly['timestamp'] = datetime.now(datetime.timezone.utc).isoformat()
