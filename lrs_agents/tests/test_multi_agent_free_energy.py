...
class TestTotalFreeEnergy:
    ...
    def test_combines_environmental_and_social(self):
        ...
        G_total = calculate_total_free_energy(policy, state, preferences, social_precisions)
    ...
    def test_high_social_uncertainty_increases_G(self):
        ...
        G_high_trust = calculate_total_free_energy(
            policy, state, preferences, social_precisions={"agent_b": 0.9}
        )

        # Low trust
        G_low_trust = calculate_total_free_energy(
            policy, state, preferences, social_precisions={"agent_b": 0.2}
        )
    ...
    def test_social_weight_parameter(self):
        ...
        G_low_weight = calculate_total_free_energy(
            policy, state, preferences, social_precisions, social_weight=0.5
        )

        G_high_weight = calculate_total_free_energy(
            policy, state, preferences, social_precisions, social_weight=2.0
        )
...
class TestCommunicationDecision:
    ...
    def test_stochastic_selection_low_precision(self):
        ...
        decisions = [
            should_communicate_based_on_G(G_communicate, G_no_communicate, precision=0.3)
            for _ in range(100)
        ]
    ...
    def test_deterministic_selection_high_precision(self):
        ...
        decisions = [
            should_communicate_based_on_G(G_communicate, G_no_communicate, precision=0.95)
            for _ in range(50)
        ]
