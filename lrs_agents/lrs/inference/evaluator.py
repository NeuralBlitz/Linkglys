...
from lrs.core.free_energy import calculate_expected_free_energy, PolicyEvaluation
...
class HybridGEvaluator:
    ...
    def __init__(self, lambda_fn: Optional[callable] = None, epistemic_weight: float = 1.0):
        """
    ...
    def _calculate_llm_g(self, proposal: Dict[str, Any], preferences: Dict[str, float]) -> float:
        """
...
def compare_math_vs_llm(
    ...
    # Mathematical G
    G_math = calculate_expected_free_energy(
        policy=policy, preferences=preferences, historical_stats=historical_stats
    )

    # LLM G
    G_llm = evaluator._calculate_llm_g(proposal, preferences)

    return {"G_math": G_math, "G_llm": G_llm, "difference": abs(G_math - G_llm)}
